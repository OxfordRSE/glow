...
def populate_neighbors(min_geographical: int = 2, min_statistical: int = 2) -> None:
    ...
    try:
        ...
        if len(schools) < 3:
            print(
                f"Warning: Only {len(schools)} schools found. Need at least 3 to create meaningful neighbor relationships."
            )
            if len(schools) < 2:
                print(
                    "Error: Need at least 2 schools to create neighbor relationships."
                )
                return

        print(f"Found {len(schools)} schools")
        print(
            f"Ensuring each school has at least {min_geographical} geographical and {min_statistical} statistical neighbors"
        )
