<script lang="ts">
  ...
  import { createI18n, initializeLocale, locale } from '$lib/i18n';
    ...
    <nav class="bg-white border-b border-gray-200 shadow-sm">
        ...
        <div class="flex h-16 items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            ...
            <span
              ...
              title={i18n.t('nav.apiStatus', { status: apiHealth })}
            >
              {#if apiHealth === 'ok'}
      ...
      {#if mobileMenuOpen}
        <div class="md:hidden border-t border-gray-200 px-4 py-3 space-y-1">
          <a href="/" class="block px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-100" onclick={() => (mobileMenuOpen = false)}>{i18n.t('nav.home')}</a>
          <a href="/query" class="block px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-100" onclick={() => (mobileMenuOpen = false)}>{i18n.t('nav.query')}</a>
          {#if $isAdmin}
            <a href="/admin" class="block px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-100" onclick={() => (mobileMenuOpen = false)}>{i18n.t('nav.admin')}</a>
          {/if}
          {#if $authStore.user}
          ...
          <div class="px-3 py-2 text-xs text-gray-400">
            {i18n.t('nav.api')}: {apiHealth === 'ok' ? i18n.t('nav.online') : apiHealth === 'down' ? i18n.t('nav.offline') : i18n.t('nav.checking')}
          </div>
        </div>
      {/if}
    </nav>
