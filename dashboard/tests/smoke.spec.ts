...
test('admin can log in, run a query, and use the admin screen', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Means Result' })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Show Table' })).toBeVisible();

  await page.goto('/admin');
  await expect(page.getByRole('heading', { name: 'User Management' })).toBeVisible();
  await expect(page.getByRole('cell', { name: adminUser, exact: true })).toBeVisible();
  await expect(page.getByRole('cell', { name: scopedUser, exact: true })).toBeVisible();
});

test('scoped user only sees their scoped school in query results', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Frequency Result' })).toBeVisible();
